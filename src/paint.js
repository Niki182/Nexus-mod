...
PaintEditorMorph.prototype.populatePropertiesMenu = function () {
    ...
    pc.penSizeSlider.action = function (num) {
        num = Math.max(1, Math.min(100, Number(num) || 1));
        if (pc.penSizeField) {
    ...
    pc.penSizeField.accept = function () {
        var val = Math.max(1, Math.min(100, parseFloat(pc.penSizeField.getValue()) || 1));
...
PaintCanvasMorph.prototype.undo = function () {
    if (this.undoBuffer.length > 0) {
        var current = newCanvas(this.extent(), true);
        this.merge(this.paper, current);
        this.redoBuffer.push(current);
...
PaintCanvasMorph.prototype.redo = function () {
    if (this.redoBuffer.length > 0) {
        var current = newCanvas(this.extent(), true);
        this.merge(this.paper, current);
        this.undoBuffer.push(current);
        this.paper = newCanvas(this.extent(), true);
        this.merge(this.redoBuffer.pop(), this.paper);
        this.drawNew();
        this.changed();
    }
};
...
PaintCanvasMorph.prototype.drawcrosshair = function (context) {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(rp.x, rp.y, 3, 0, radians(360), false);
    ctx.fill();
    ctx.stroke();
...
PaintCanvasMorph.prototype.floodfill = function (sourcepoint) {
    var width = this.paper.width,
        ...
        data = img.data,
        x = Math.max(0, Math.min(width - 1, Math.round(sourcepoint.x))),
        y = Math.max(0, Math.min(height - 1, Math.round(sourcepoint.y))),
        stack = [y * width + x],
    ...
    while (stack.length > 0) {
        ...
        if (visited[currentpoint] || !checkpoint(read(currentpoint))) {
            continue;
        }
        visited[currentpoint] = 1;
        if (currentpoint % width < width - 1) { stack.push(currentpoint + 1); }
        if (currentpoint % width > 0) { stack.push(currentpoint - 1); }
        if (currentpoint >= width) { stack.push(currentpoint - width); }
        if (currentpoint < width * (height - 1)) { stack.push(currentpoint + width); }
