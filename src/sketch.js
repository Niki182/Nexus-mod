...
VectorPaintEditorMorph.prototype.init = function () {
    ...
    this.redoHistory = []; // shapes history, for redo purposes
...
VectorPaintEditorMorph.prototype.undo = function () {
	...
	while (this.history.length && oldSum == newSum) {
        this.redoHistory.push(this.shapes.map(function (shape) {
            return shape.copy();
        }));
...
VectorPaintEditorMorph.prototype.redo = function () {
    var paper = this.paper,
        oldSum = this.checksum(),
        newSum = oldSum,
        current;

    while (this.redoHistory.length && oldSum == newSum) {
        current = this.shapes.map(function (shape) {
            return shape.copy();
        });
        this.history.push(current);
        this.shapes = this.redoHistory.pop();
        this.shapes.forEach(function (shape) {
            shape.drawOn(paper);
        });
        newSum = this.checksum();
